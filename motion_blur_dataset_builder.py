...
try:
    ...
    from haze_dataset_builder import _load_rgb_tensor, _save_rgb_tensor
...
def _resolve_runtime_device(device: str) -> torch.device:
    if not isinstance(device, str) or device.strip() == "":
        raise ValueError(f"device must be non-empty string, got {device!r}")
    ...
    if name.startswith("cuda"):
        ...
        if dev.index is not None and dev.index >= torch.cuda.device_count():
            raise ValueError(f"requested CUDA index {dev.index} out of range (count={torch.cuda.device_count()})")
        return dev
    raise ValueError(f"unsupported device: {device!r}. use cpu/cuda/cuda:N/auto")
